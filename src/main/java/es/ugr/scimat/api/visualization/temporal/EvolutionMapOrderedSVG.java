 ...
 */
package es.ugr.scimat.api.visualization.temporal;
...
import es.ugr.scimat.api.analysis.temporal.EvolutionMapNexus;

/**
 *
 * @author mjcobo
 */
public class EvolutionMapOrderedSVG {
  ...
  public EvolutionMapOrderedSVG(int minRX, int maxRX, 
  ...
  public Document buildXML(ArrayList<ClusterSet> clusterSets, EvolutionMap evolutionMap,
                           ArrayList<ArrayList<Integer>> orderedClustersPerPeriod,
    ...
    if (clusterSets.size() > 0) {
      ...
      lastCoords = drawCluster(clusterSets.get(0), 
              orderedClustersPerPeriod.get(0), 
              0, 
              maxClusterSize,
              volumePropertyName, 
              labelPropertyName, 
      ...
      for (i = 1; i < clusterSets.size(); i++) {
        ...
        currentCoords = drawCluster(clusterSets.get(i),
                orderedClustersPerPeriod.get(i), 
                i, 
                maxClusterSize,
                volumePropertyName,
                labelPropertyName,
   ...
   * @return 
   */
  private double[][] drawCluster(ClusterSet clusterSet, ArrayList<Integer> orderedClusterId, int period, 
    ...
    for (i = 0; i < orderedClusterId.size(); i++) {
      ...
      if ((volumePropertyName == null) || (maxClusterSize == 0.0)) {
        ...
        volume = (Double) clusterSet.getCluster(orderedClusterId.get(i)).getProperties().getProperty(volumePropertyName).getValue();
        volume = ((volume / maxClusterSize) * (maxRX - minRX)) + minRX;
      }
      
      coords[orderedClusterId.get(i)][0] = x;
      coords[orderedClusterId.get(i)][1] = y;
      coords[orderedClusterId.get(i)][2] = volume;
      ...
      subElement.setTextContent((String) clusterSet.getCluster(orderedClusterId.get(i)).getProperties().getProperty(labelPropertyName).getValue());
      element.appendChild(subElement);
    }
   ...
   * @param element 
   */
  private void drawArrow(double x1, double y1, double x2, double y2, double nexusWeight, 
          boolean solidLine, Document doc, String svgNS, Element element) {
